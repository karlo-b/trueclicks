...
/**
 * Shows a breadcrumb for all types of pages.  This is a wrapper function for the XT_Breadcrumbs class,
 * which should be used in theme templates.
 *
 * @since  0.1.0
 * @access public
 * @param  array $args Arguments to pass to XT_Breadcrumbs.
 * @return void
 */
function xt_do_breadcrumbs( $args = array() ) {
	...
	if ( is_array( $excluded_breadcrumbs ) && ! empty( $excluded_breadcrumbs ) ) {
		...
		foreach ( $excluded_breadcrumbs as $excluded_breadcrumb ) {
	...
	if ( ! is_object( $breadcrumb ) )
		$breadcrumb = new XT_Breadcrumbs( $args );
 ...
 */
function xt_breadcrumbs_content() {
...
add_action( 'xt_inner_content_open', 'xt_breadcrumbs_content' );

/**
 * Add Breadcrumbs Below Header
 */
function xt_breadcrumbs_header() {
...
add_action( 'xt_after_header', 'xt_breadcrumbs_header' );
...
class XT_Breadcrumbs {
	...
	/**
	 * Sets up the breadcrumb trail properties.  Calls the `XT_Breadcrumbs::add_items()` method
	 ...
	 *     @type array     $labels         Text labels. @see XT_Breadcrumbs::set_labels()
	 *     @type array     $post_taxonomy  Taxonomies to use for post types. @see XT_Breadcrumbs::set_post_taxonomy()
	...
	public function __construct( $args = array() ) {
		...
		$defaults = array(
			...
			'list_tag'        => 'ul',
			'item_tag'        => 'li',
			'separator'		  => apply_filters( 'xt_breadcrumbs_separator', '/' ),
			'show_on_front'   => true,
			'network'         => false,
			'show_title'      => true,
			'labels'          => array(),
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			$breadcrumb .= sprintf(
				'<%s class="xt-breadcrumbs" itemscope="itemscope" itemtype="https://schema.org/BreadcrumbList">',
			...
			foreach ( $this->items as $item ) {
				...
				$item_class = 'xt-breadcrumb';

				if ( 1 === $item_position && 1 < $item_count ) {
					$item_class .= ' xt-breadcrumb-first';
					$separator = $this->args['separator'];

				} elseif ( $item_count === $item_position ) {
					$item_class .= ' xt-breadcrumb-last';
					$separator = '';
				}

				// Create list item attributes.
				$attributes = 'itemprop="itemListElement" itemscope="itemscope" itemtype="https://schema.org/ListItem" class="' . $item_class . '"';
			...
			$breadcrumb = sprintf(
				'<%1$s aria-label="%2$s" class="xt-breadcrumbs-wrapper" itemprop="breadcrumb">%3$s%4$s%5$s</%1$s>',
		...
		$breadcrumb = apply_filters( 'xt_breadcrumbs', $breadcrumb, $this->args );
		...
		if( get_theme_mod( 'breadcrumbs_position' ) === 'header' ) {
			...
			$output  = '<div class="xt-breadcrumbs-container">';
			$output .= '<div class="xt-container xt-container-center">';
			$output .= $breadcrumb;
			$output .= '</div>';
			$output .= '</div>';
	...
	protected function set_labels() {
		...
		$defaults = array(
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'xt-framework' ),
			'home'                => esc_html__( 'Home', 'xt-framework' ),
			'error_404'           => esc_html__( '404 Not Found', 'xt-framework' ),
			'archives'            => esc_html__( 'Archives', 'xt-framework' ),
			// Translators: %s is the search query.
			'search'              => esc_html__( 'Search results for: %s', 'xt-framework' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s', 'xt-framework' ),
			// Translators: %s is the page number.
			'paged_comments'      => esc_html__( 'Comment Page %s', 'xt-framework' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s', 'xt-framework' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s', 'xt-framework' ),
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( 'g:i a' ) );
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( 'i' ) );
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( 'g a' ) );
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'],  get_the_time( 'Y' ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( 'F' ) );
		$day   = sprintf( $this->labels['archive_day'],   get_the_time( 'j' ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'],  get_the_time( 'Y' ) );
		$week = sprintf( $this->labels['archive_week'],  get_the_time( 'W' ) );
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( 'Y' ) ) );

				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( 'F' ) ) );

				// If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( 'j' ) ) );
